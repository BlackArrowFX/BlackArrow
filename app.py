...
with st.sidebar:
    ...
    if st.button("✅ RECORD WIN", disabled=loss_disabled, use_container_width=True):
...
bias_5m_ok = c5m.checkbox("5M Confirmed", disabled=not (s5_h > 0 and s5_l > 0) or not bias_1h_ok)
...
with col_poi:
    ...
    poi_type = st.selectbox("Where am I trading?", ["Select POI...", "Swing High", "Swing Low", "Supply Zone", "Demand Zone", "Order Block", "FVG / Imbalance"], disabled=not bias_5m_ok)
